...
// node_modules/@photo-sphere-viewer/gallery-plugin/index.module.js
...
var _ShowGalleryEvent = class _ShowGalleryEvent2 extends TypedEvent {
  /** @internal */
  constructor(fullscreen) {
    super(_ShowGalleryEvent2.type);
    this.fullscreen = fullscreen;
  }
};
_ShowGalleryEvent.type = "show-gallery";
var ShowGalleryEvent = _ShowGalleryEvent;
var _HideGalleryEvent = class _HideGalleryEvent2 extends TypedEvent {
  /** @internal */
  constructor() {
    super(_HideGalleryEvent2.type);
  }
};
...
var GALLERY_ITEM_DATA_KEY = utils_exports.dasherize(GALLERY_ITEM_DATA);
...
var GalleryComponent = class extends AbstractComponent {
  constructor(plugin, viewer) {
    super(viewer, {
      className: `psv-gallery ${constants_exports.CAPTURE_EVENTS_CLASS}`
    });
    this.plugin = plugin;
    this.state = {
      visible: true,
    ...
    closeBtn.innerHTML = constants_exports.ICONS.close;
    ...
    this.state.itemMargin = parseInt(utils_exports.getStyleProperty(this.items, "padding-left"), 10);
    this.state.breakpoint = parseInt(utils_exports.getStyleProperty(this.container, "--psv-gallery-breakpoint"), 10);
    ...
    this.viewer.addEventListener(events_exports.SizeUpdatedEvent.type, this);
  ...
  handleEvent(e) {
    switch (e.type) {
      ...
      case "keydown":
        if (this.isVisible()) {
          switch (e.key) {
            case constants_exports.KEY_CODES.Escape:
              this.plugin.hide();
              break;
            case constants_exports.KEY_CODES.Enter:
              this.__click(e);
              break;
          }
        }
  ...
  hide() {
    ...
    if (utils_exports.hasParent(document.activeElement, this.container)) {
   ...
   */
  __click(e) {
    const item = utils_exports.getMatchingTarget(e, `.psv-gallery-item`);
    if (!item) {
      return;
    }
  ...
  __updateArrows() {
    ...
    if (this.state.isAboveBreakpoint !== this.isAboveBreakpoint) {
      utils_exports.toggleClass(this.arrowLeft, "psv-gallery-arrow--left", this.isAboveBreakpoint);
      utils_exports.toggleClass(this.arrowLeft, "psv-gallery-arrow--top", !this.isAboveBreakpoint);
      utils_exports.toggleClass(this.arrowRight, "psv-gallery-arrow--right", this.isAboveBreakpoint);
      utils_exports.toggleClass(this.arrowRight, "psv-gallery-arrow--bottom", !this.isAboveBreakpoint);
...
var _GalleryPlugin = class _GalleryPlugin2 extends AbstractConfigurablePlugin {
  ...
  init() {
    super.init();
    utils_exports.checkStylesheet(this.viewer.container, "gallery-plugin");
    this.map = this.viewer.getPlugin("map");
    this.plan = this.viewer.getPlugin("plan");
    this.viewer.addEventListener(events_exports.PanoramaLoadedEvent.type, this);
    this.viewer.addEventListener(events_exports.ShowPanelEvent.type, this);
    if (this.config.visibleOnLoad) {
      this.viewer.addEventListener(events_exports.ReadyEvent.type, () => {
   ...
   */
  destroy() {
    this.viewer.removeEventListener(events_exports.PanoramaLoadedEvent.type, this);
   ...
   */
  handleEvent(e) {
    if (e instanceof events_exports.PanoramaLoadedEvent) {
      const item = this.items.find((i) => utils_exports.deepEqual(i.panorama, e.data.panorama));
      this.currentId = item == null ? void 0 : item.id;
      this.gallery.setActive(this.currentId);
    } else if (e instanceof events_exports.ShowPanelEvent) {
  ...
  show() {
    var _a, _b;
    (_a = this.map) == null ? void 0 : _a.minimize();
    (_b = this.plan) == null ? void 0 : _b.minimize();
  ...
  __updateButton() {
    var _a;
    (_a = this.viewer.navbar.getButton(GalleryButton.id, false)) == null ? void 0 : _a.toggle(this.items.length > 0);
  }
};
...
@photo-sphere-viewer/gallery-plugin/index.module.js:
  (*!
   ...
   *)
*/
//# sourceMappingURL=@photo-sphere-viewer_gallery-plugin.js.map
